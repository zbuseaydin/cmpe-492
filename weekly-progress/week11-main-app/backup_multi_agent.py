...
class MultiAgents:
    ...
    def agent_node(self, agent_id: int):
        ...
        def node_function(state):
            ...
            current_context = "\n".join([
                f"Previous discussion:" if messages else "",
                *[f"Agent {m['agent']}: {m['content']}" for m in messages],
            ...
            left_desc = json.dumps({
                ...
                "members": [{"type": k, "count": v} for k, v in scenario.left.items()]
            }, indent=4)
            
            right_desc = json.dumps({
                "total number of fatalities": right_group_size,
                "members": [{"type": k, "count": v} for k, v in scenario.right.items()]
            }, indent=4)
    ...
    def host_node(self, state):
        ...
        if len(recent_decisions) == 3:
            if all(d == recent_decisions[0] for d in recent_decisions) or len(state["messages"]) >= 6:
                return {"messages": state["messages"], "scenario": state["scenario"], "next": END}
